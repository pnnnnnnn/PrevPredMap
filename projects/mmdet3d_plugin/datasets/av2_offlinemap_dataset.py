...
class CustomAV2OfflineLocalMapDataset(CustomNuScenesDataset):
    ...
    def get_data_info(self, index):
        ...
        if 'prev_idx_list' in info.keys():
            if len(info['prev_idx_list']) < self.prev_interval:
                prev_idx = ''
            else:
                prev_idx = str(info['prev_idx_list'][-self.prev_interval]).zfill(16)
        else:
            prev_idx = ''
        ...
        input_dict = dict(
            ...
            sample_idx=str(info['sample_idx']).zfill(16),
            prev_idx=prev_idx,
        )
