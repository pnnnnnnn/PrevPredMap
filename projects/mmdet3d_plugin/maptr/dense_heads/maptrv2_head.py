...
from projects.mmdet3d_plugin.maptr.modules.position_encoding import pos2posemb2d, pos2posemb3d
import random
...
class MapTRv2Head(DETRHead):
    ...
    def __init__(self,
        ...
        self.topk_prev_pred = topk_prev_pred
        if self.use_prev_pred:
            self.label_enc = nn.Embedding(self.cls_out_channels, self.embed_dims)
            self.score_enc = nn.Sequential(
                Linear(1, self.embed_dims),
        ...
        self.num_vec_one2one_origin = self.num_vec_one2one
    ...
    def _init_layers(self):
        ...
        if not self.as_two_stage:
            ...
            elif self.query_embed_type == 'instance_pts_v5':
                self.instance_embedding_content = nn.Embedding(self.num_vec, self.embed_dims)
                self.instance_embedding_ref = nn.Embedding(self.num_vec, self.code_size)
                self.pts_embedding_content = nn.Embedding(self.num_pts_per_vec, self.embed_dims)
                self.pts_embedding_ref = nn.Embedding(self.num_pts_per_vec, self.code_size)
                self.ref2pos = nn.Sequential(
                    nn.Linear(self.embed_dims * self.code_size // 2, self.embed_dims),
                    nn.ReLU(),
                    nn.Linear(self.embed_dims, self.embed_dims)
                )
                self.transformer.decoder.ref2pos = self.ref2pos
            else:
    ...
    def forward(self, mlvl_feats, lidar_feat, img_metas, prev_bev=None, only_bev=False, prev_pred_info=None):
        """Forward function.
        Args:
        ...
        """
        self.num_vec_one2one = self.num_vec_one2one_origin
        if self.training:
        ...
        if self.query_embed_type == 'all_pts':
            ...
            instance_embeds_content = self.instance_embedding_content.weight[0:num_vec].unsqueeze(1)
            pts_embedding_content = self.pts_embedding_content.weight[0:self.num_pts_per_vec]
            pts_embeds_content = pts_embedding_content.view(1, self.num_pts_per_vec, -1).repeat(num_vec, 1, 1)
            object_query_embeds_content = (instance_embeds_content + pts_embeds_content).flatten(0, 1).to(dtype)
            instance_embeds_ref = self.instance_embedding_ref.weight[0:num_vec].unsqueeze(1)
            pts_embedding_ref = self.pts_embedding_ref.weight[0:self.num_pts_per_vec]
            pts_embeds_ref = pts_embedding_ref.view(1, self.num_pts_per_vec, -1).repeat(num_vec, 1, 1)
            object_query_embeds_ref = (instance_embeds_ref + pts_embeds_ref).flatten(0, 1).to(dtype)
            object_query_embeds_ref = object_query_embeds_ref.sigmoid()
            if self.code_size == 2:
                object_query_embeds_pos = self.ref2pos(pos2posemb2d(object_query_embeds_ref.view(1, -1, 2)))
            elif self.code_size == 3:
                object_query_embeds_pos = self.ref2pos(pos2posemb3d(object_query_embeds_ref.view(1, -1, 3)))
            else:
                raise NotImplementedError("code size should be 2 or 3")
            object_query_embeds = torch.cat([object_query_embeds_pos[0], object_query_embeds_content], dim=-1)
        else:
            raise NotImplementedError
        ...
        object_query_embeds = object_query_embeds.unsqueeze(0).expand(bs, -1, -1)
        if self.query_embed_type == 'instance_pts_v5':
            object_query_embeds_ref = object_query_embeds_ref.unsqueeze(0).expand(bs, -1, -1)
        
        if prev_pred_info is not None:
            list_labels = []
            list_vectors = []
            list_scores = []
            list_vectors_current = []
            for i, prev_pred in enumerate(prev_pred_info):
                list_labels.append(prev_pred['labels'])
                list_vectors.append(prev_pred['vectors'])
                list_scores.append(prev_pred['scores'])
                # global to current ego and normalize
                l2g = torch.Tensor(img_metas[i]['lidar2global']).to(device)
                l2g_R, l2g_T = l2g[:3, :3], l2g[:3, -1].view(3, 1)
                l2e = torch.Tensor(img_metas[i]['lidar2ego']).to(device)
                l2e_R, l2e_T = l2e[:3, :3], l2e[:3, -1].view(3, 1)
                g2l_R, g2l_T = l2g_R.T, -l2g_R.T.mm(l2g_T)
                g2e_R, g2e_T = l2e_R.mm(g2l_R), l2e_R.mm(g2l_T) + l2e_T
                
                num_i, num_p, _ = prev_pred['vectors_global'].shape
                num_c = self.code_size
                vectors_ego = (g2e_R.mm(prev_pred['vectors_global'].view(-1, 3).T) + g2e_T).T
                vectors_ego = vectors_ego[:, :num_c].view(num_i, num_p, num_c)

                if num_c == 2:
                    vectors_ego_norm = normalize_2d_pts(vectors_ego, self.pc_range)
                elif num_c == 3:
                    vectors_ego_norm = normalize_3d_pts(vectors_ego, self.pc_range)
                else:
                    raise NotImplementedError("code size should be 2 or 3")
                vectors_ego_norm = vectors_ego_norm.clamp(min=0.0, max=1.0)
                list_vectors_current.append(vectors_ego_norm)
            
            prev_labels = torch.cat(list_labels).view(bs, -1).to(device)
            prev_vectors = torch.cat(list_vectors).view(bs, -1, self.code_size).to(device)
            prev_scores = torch.cat(list_scores).view(bs, -1).to(device)
            prev_vectors_current = torch.cat(list_vectors_current).view(bs, -1, self.code_size).to(device)
            self.topk_prev_pred = prev_labels.shape[1]
            prev_vectors = prev_vectors_current
            object_query_embeds_pos, object_query_embeds_content \
                = torch.split(object_query_embeds, self.embed_dims, dim=2)
            assert self.query_embed_type == 'instance_pts_v5'
            assert self.num_group > 0
            if self.training:
                object_query_embeds_pos_tp = torch.split(object_query_embeds_pos, \
                                                            self.num_vec_one2one * self.num_pts_per_vec, dim=1)
                object_query_embeds_content_tp = torch.split(object_query_embeds_content, \
                    self.num_vec_one2one * self.num_pts_per_vec, dim=1)
                object_query_embeds_ref_tp = torch.split(object_query_embeds_ref, \
                    self.num_vec_one2one * self.num_pts_per_vec, dim=1)
                object_query_embeds_pos_list = list(object_query_embeds_pos_tp)
                object_query_embeds_content_list = list(object_query_embeds_content_tp)
                object_query_embeds_ref_list = list(object_query_embeds_ref_tp)
                assert len(object_query_embeds_pos_list) == (self.num_group + 1)
                for i in range(self.num_group + 1):
                    prev_labels_embed = self.label_enc(copy.deepcopy(prev_labels))
                    prev_scores_embed = self.score_enc(copy.deepcopy(prev_scores.unsqueeze(-1)))
                    prev_vectors_tmp = copy.deepcopy(prev_vectors)
                    if self.code_size == 2:
                        prev_pos = self.ref2pos(pos2posemb2d(prev_vectors_tmp))
                    elif self.code_size == 3:
                        prev_pos = self.ref2pos(pos2posemb3d(prev_vectors_tmp))
                    else:
                        raise NotImplementedError("code size should be 2 or 3")
                    prev_labels_embed = prev_labels_embed + prev_scores_embed
                    prev_content = prev_labels_embed.view(bs, self.topk_prev_pred, 1, self.embed_dims) \
                        + pts_embeds_content[0].unsqueeze(0).repeat(bs, self.topk_prev_pred, 1, 1)
                    prev_content = prev_content.view(bs, -1, self.embed_dims)
                    object_query_embeds_content_cp = object_query_embeds_content_list[i].clone()
                    object_query_embeds_ref_cp = object_query_embeds_ref_list[i].clone()
                    object_query_embeds_pos_cp = object_query_embeds_pos_list[i].clone()
                    object_query_embeds_content_cp[:, :self.topk_prev_pred * self.num_pts_per_vec, :] = prev_content
                    object_query_embeds_ref_cp[:, :self.topk_prev_pred * self.num_pts_per_vec, :] = prev_vectors_tmp
                    object_query_embeds_pos_cp[:, :self.topk_prev_pred * self.num_pts_per_vec, :] = prev_pos
                    object_query_embeds_content_list[i] = object_query_embeds_content_cp
                    object_query_embeds_ref_list[i] = object_query_embeds_ref_cp
                    object_query_embeds_pos_list[i] = object_query_embeds_pos_cp
                object_query_embeds_pos = torch.cat(object_query_embeds_pos_list, dim=1)
                object_query_embeds_content = torch.cat(object_query_embeds_content_list, dim=1)
                object_query_embeds_ref = torch.cat(object_query_embeds_ref_list, dim=1)
            else:
                prev_labels_embed = self.label_enc(copy.deepcopy(prev_labels))
                prev_scores_embed = self.score_enc(copy.deepcopy(prev_scores.unsqueeze(-1)))
                prev_labels_embed = prev_labels_embed + prev_scores_embed
                if self.code_size == 2:
                    prev_pos = self.ref2pos(pos2posemb2d(prev_vectors))
                elif self.code_size == 3:
                    prev_pos = self.ref2pos(pos2posemb3d(prev_vectors))
                else:
                    raise NotImplementedError("code size should be 2 or 3")
                prev_content = prev_labels_embed.view(bs, self.topk_prev_pred, 1, self.embed_dims) \
                    + pts_embeds_content[0].unsqueeze(0).repeat(bs, self.topk_prev_pred, 1, 1)
                prev_content = prev_content.view(bs, -1, self.embed_dims)
                object_query_embeds_content[:, :self.topk_prev_pred * self.num_pts_per_vec, :] = prev_content
                object_query_embeds_ref[:, :self.topk_prev_pred * self.num_pts_per_vec, :] = prev_vectors
                object_query_embeds_pos[:, :self.topk_prev_pred * self.num_pts_per_vec, :] = prev_pos
            object_query_embeds = torch.cat([object_query_embeds_pos, object_query_embeds_content], dim=2)
        ...
        if only_bev:  # only use encoder to obtain BEV features, TODO: refine the workaround
            ...
            outputs = self.transformer(
                ...
                init_ref_pts=object_query_embeds_ref if self.query_embed_type =='instance_pts_v5' else None,
        )

        bev_embed,depth, hs, init_reference, inter_references = outputs
        init_reference = init_reference.view(init_reference.shape[0], -1, self.code_size)
        inter_references = inter_references.view(inter_references.shape[0], \
                                                 inter_references.shape[1], -1, self.code_size)
        ...
        for lvl in range(hs.shape[0]):
            if self.query_embed_type == 'all_refs':
                if lvl == 0:
                    reference = refs_embeds.repeat(bs, 1, 1)
                    reference = reference.view(reference.shape[0], -1, self.code_size)
                tmp = self.reg_branches[lvl](hs[lvl])
                tmp = tmp.view(tmp.shape[0], -1, self.code_size)
                tmp = tmp[..., 0:2] if not self.z_cfg['gt_z_flag'] else tmp[..., 0:3]
                new_reference = tmp + inverse_sigmoid(reference)
                new_reference = new_reference.sigmoid()
                reference = new_reference.detach()
                tmp = new_reference    # look forward twice
            elif self.query_embed_type == 'instance_pts_v5':
                if lvl == 0:
                    # reference = object_query_embeds_ref.unsqueeze(0).repeat(bs, 1, 1)
                    reference = object_query_embeds_ref
                    reference = reference.view(reference.shape[0], -1, self.code_size)
                tmp = self.reg_branches[lvl](hs[lvl])
                tmp = tmp.view(tmp.shape[0], -1, self.code_size)
                tmp = tmp[..., 0:2] if not self.z_cfg['gt_z_flag'] else tmp[..., 0:3]
