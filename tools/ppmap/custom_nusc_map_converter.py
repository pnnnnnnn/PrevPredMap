...
def create_nuscenes_infos(root_path,
    ...
    if test:
        ...
        list_prev_idx = []
        log_id_prev = None
        for idx, sample in enumerate(data['infos']):
            if sample['scene_token'] == log_id_prev:
                sample['prev_idx_list'] = copy.deepcopy(list_prev_idx)
                list_prev_idx.append(sample['token'])
            else:
                sample['prev_idx_list'] = []
                list_prev_idx = [sample['token']]
            log_id_prev = sample['scene_token']
        mmcv.dump(data, info_path)
